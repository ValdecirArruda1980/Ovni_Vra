...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* 1. Bola Laranja (Luminoso / Plasma) */
        .shape-plasma {
            width: 14px;
            height: 14px;
...
<body>
    ...
    <script>
        ...
        function getShapeConfig(shape, isNasa) {
            if (isNasa) return { css: 'shape-nasa', name: 'Artefato / Meteoro', iconSymbol: '☄️' };
            
            const s = (shape || '').toLowerCase();
            if (s.includes('triângulo') || s.includes('triangulo')) return { css: 'shape-triangle', name: 'Triângulo Anômalo', iconSymbol: '▲' };
            if (s.includes('esfera') || s.includes('orb') || s.includes('luminosa')) return { css: 'shape-plasma', name: 'Esfera Luminosa (Plasma)', iconSymbol: '🟠' };
            if (s.includes('charuto')) return { css: 'shape-charuto', name: 'Charuto Tático', iconSymbol: '🔴' };
            if (s.includes('tic-tac') || s.includes('cilindro')) return { css: 'shape-cylinder', name: 'Cilindro Tático', iconSymbol: '🔋' };
            if (s.includes('disco') || s.includes('nave')) return { css: 'shape-nave', name: 'Nave Metálica', iconSymbol: '🛸' };
...
print("✅ Ícones geométricos personalizados aplicados com sucesso!")
