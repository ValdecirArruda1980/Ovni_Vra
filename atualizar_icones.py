...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Estilos dos Ícones Customizados do Mapa */
        .custom-ufo-icon {
            display: flex;
            align-items: center;
...
<body>
    ...
    <script>
        ...
        function getIconByShape(shape, isNasa) {
        ...
        async function loadUfoData() {
            ...
            try {
                ...
                result.data.forEach((item, idx) => {
                    const isNasa = item.source.includes("NASA");
                    const iconData = getIconByShape(item.shape, isNasa);
                    const keyId = `ufo-${idx}`;
                    ...
                    if (item.latitude && item.longitude) {
                        ...
                        const popupContent = `
                            <b style="color:${isNasa ? '#ff0055' : '#00ffaa'}">${iconData.icon} ${item.id}</b><br>
                            ...
                            <b>FORMA/TIPO:</b> ${iconData.icon} ${item.shape}<br>
                    ...
                    card.innerHTML = `
                        <div class="ufo-title">
                            <span>${iconData.icon} ${item.id}</span>
                        ...
                        <b>FORMA:</b> ${iconData.icon} ${item.shape}<br>
                        <b>VELOCIDADE:</b> ${item.speed_kmh.toLocaleString()} km/h<br>
                        <b>ALTITUDE:</b> ${item.altitude_m.toLocaleString()} m<br>
